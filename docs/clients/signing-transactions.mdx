...
// broadcast a sign transactions request
...
import { BaseARC0027Error, ISignTransactionsResult } from '@agoralabs-sh/avm-web-provider';
...
// initialized client
client.onSignTransactions((result: ISignTransactionsResult: null, error: BaseARC0027Error | null) => {
  if (error) {
    console.error('error:', error);
