...
In order to prevent provider collisions, the client instantiates an event listener and emits an event. The provider listens to any client's incoming events and upon receiving an event, emits an event in response. This forms an event concurrency loop.

Since the client & provider code aren’t guaranteed to run in a particular order, the events are designed to handle such race conditions.
