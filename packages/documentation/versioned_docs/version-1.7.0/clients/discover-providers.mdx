...
The first thing we want to do, after [initialization](./getting-started.mdx), is start listening to discover messages. The callback passed will be called everytime a `discover` message is responded to by a provider. However, before we receive any `discover` messages we need then broadcast to any providers a request.
