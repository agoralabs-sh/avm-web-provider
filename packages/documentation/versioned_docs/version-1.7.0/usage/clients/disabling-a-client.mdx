...
Assuming a client has been [enabled](./enabling-a-client.mdx) with one or all providers, you can remove the client from all available providers by:
...
If any providers do not support the `disable`  method, then a [`MethodNotSupportedError`](../../api-reference/errors.md#methodnotsupportederror) should be returned.
